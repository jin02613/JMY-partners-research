[main.py](https://github.com/user-attachments/files/25651655/main.py)
# -*- coding: utf-8 -*-
"""
기업 재무분석 대시보드 서버
- Flask 기반 웹 서버
...
def fetch_quarterly_data(company_name, start_year, end_year, fs_pref='CFS'):
    ...
    # Phase 1.5: 이상치 보정 — DART 공시 단위 오류(백만원↔원) 감지 시 /1e6 보정
    for item in TARGET_ITEMS:
        abs_vals = []
        for qtr in quarterly_results.values():
            v = qtr.get(item)
        ...
        if len(abs_vals) < 3:
            continue
        abs_vals.sort()
        median_val = abs_vals[len(abs_vals) // 2]
        if median_val == 0:
            continue
        threshold = median_val * 1000
        for qtr in quarterly_results.values():
            v = qtr.get(item)
            if v is not None and abs(v) > threshold:
                qtr[item] = round(v / 1e6)

    # Phase 2: Q4 계산 (보정된 Q1-Q3 사용)
    for year in years:
        label = f"{year}Q4"
        data = raw_data.get((year, 'Q4'))
        if data is None:
            continue
        qtr_data = {}
        for item_name in TARGET_ITEMS:
            ...
            q123_count = 0
            for q in ['Q1', 'Q2', 'Q3']:
                v = quarterly_results.get(f"{year}{q}", {}).get(item_name)
                if v is not None:
                    q123_sum += v
                    q123_count += 1
            if q123_count >= 1:
...
''' + get_기업개요_html() + '''
  ...
  <div class="tab-content" id="content-chart">
    ...
    <div class="controls">
      ...
      <button onclick="chartDataLoaded=false; finDataLoaded=false; finData=null; loadData(false).then(()=>loadFinDataForChart())" style="
...
async function loadData(refresh = false) {
  ...
  const isBackground = finDataLoaded && finData && !refresh;
  const _showFn = refresh ? showLoading : (isBackground ? (() => {}) : showChartLoading);
  const _hideFn = refresh ? hideLoading : (isBackground ? (() => {}) : hideChartLoading);
...
function setMode(mode) {
  ...
  if (Object.keys(rawData).length > 0 || (finDataLoaded && finData)) buildChart(mode);
}
...
function setFs(fs) {
  currentFs = fs;
  if (typeof currentDataFs !== 'undefined') currentDataFs = fs;
  // fs 전환 시 finData 초기화 → rawData fallback으로 즉시 차트 표시
  finData = null;
  finDataLoaded = false;
...
async function switchTab(tabName) {
  ...
  if (tabName === 'chart') {
    await loadFinDataForChart();  // finData 우선 → 차트 즉시 표시
    if (!chartDataLoaded) loadData();  // priceData 백그라운드 로드
  }
...
async function loadFinDataForChart() {
  ...
  showChartLoading(currentCompany + ' 재무데이터를 불러오는 중...');
  await loadFinData();
  buildChart(currentMode);
  hideChartLoading();
}
